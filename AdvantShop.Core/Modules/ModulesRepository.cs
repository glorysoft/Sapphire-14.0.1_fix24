...
namespace AdvantShop.Core.Modules
{
    public class ModulesRepository
    {
        ...
        public static void InstallModuleToDb(Module module)
        {
            ...
            Debug.Log.InfoFormat("Module {0} is installed by userId: {1} from ip: {2} and user agent: {3}",
                module.StringId, CustomerContext.CurrentCustomer?.Id.ToString() ?? "NO_DATA",
                request?.UserHostAddress ?? "NO_DATA",
                request?.UserAgent ?? "NO_DATA");
        }
        ...
        public static void UninstallModuleFromDb(string moduleStringId)
        {
            ...
            HttpContext.Current.TryGetRequest(out var request);
            Debug.Log.InfoFormat("Module {0} is uninstalled by userId: {1} from ip: {2} and user agent: {3}",
                moduleStringId, CustomerContext.CurrentCustomer?.Id.ToString() ?? "NO_DATA",
                request?.UserHostAddress ?? "NO_DATA",
                request?.UserAgent ?? "NO_DATA");
        }
        ...
        public static void SetActiveModule(string moduleStringId, bool active)
        {
            ...
            HttpContext.Current.TryGetRequest(out var request);
            Debug.Log.InfoFormat("Module {0} is set activity to {4} by userId: {1} from ip: {2} and user agent: {3}",
                moduleStringId, CustomerContext.CurrentCustomer?.Id.ToString() ?? "NO_DATA",
                request?.UserHostAddress ?? "NO_DATA",
                request?.UserAgent ?? "NO_DATA",
                active);
        ...
        public static void SetModuleNeedUpdate(string stringId, bool needUpdate)
        {
            ...
            HttpContext.Current.TryGetRequest(out var request);
            Debug.Log.InfoFormat("Module {0} is set to be updated by userId: {1} from ip: {2} and user agent: {3}",
                stringId, CustomerContext.CurrentCustomer?.Id.ToString() ?? "NO_DATA",
                request?.UserHostAddress ?? "NO_DATA",
                request?.UserAgent ?? "NO_DATA");
        }
