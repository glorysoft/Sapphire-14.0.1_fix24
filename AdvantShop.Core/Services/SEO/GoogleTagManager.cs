    ...
    public class GoogleTagManager
    {
        ...
        public string RenderCounter()
        {
            ...
            if (PageType == ePageType.category || PageType == ePageType.product)
            {
                ...
                sendData.AppendFormat("'catCurrentName' : '{0}', ", StringHelper.HtmlEncode(CatCurrentName));
            }

            if (PageType == ePageType.category)
            {
                sendData.AppendFormat("'catParentId' : '{0}', ", CatParentId);
                sendData.AppendFormat("'catParentName' : '{0}', ", StringHelper.HtmlEncode(CatParentName));
            }

            if (PageType == ePageType.product)
            {
                sendData.AppendFormat("'prodId' : '{0}',", StringHelper.HtmlEncode(UseOfferId ? ProdId : ProdArtno));
                sendData.AppendFormat("'prodName' : '{0}', ", StringHelper.HtmlEncode(ProdName));
                ...
                || PageType == ePageType.purchase))
            {
                ...
                {
                    sendData.AppendFormat("'prodIds' : [{0}], ", ProdIds.Select(id => "'" + StringHelper.HtmlEncode(id) + "'").AggregateString(","));
                }
                if(!UseOfferId && ProdArtnos != null && ProdArtnos.Any())
                {
                    sendData.AppendFormat("'prodIds' : [{0}], ", ProdArtnos.Select(artno => "'" + StringHelper.HtmlEncode(artno) + "'").AggregateString(","));
                }
            }
            ...
            if ((PageType == ePageType.cart || PageType == ePageType.order) && ProdIds != null && ProdIds.Any())
            {
                ...
                sendData.AppendFormat("'prodIds' : [{0}], 'products' : [{1}],", ProdIds.Select(id => "'" + StringHelper.HtmlEncode(id) + "'").AggregateString(","), product ?? "null");
            }
                ...
                sendData.AppendFormat("'transactionProducts' : [{0}], ",
                                      Transaction.TransactionProducts.Select(
                                          ...
                                          string.Format(
                                              ...
                                              StringHelper.HtmlEncode(p.SKU), StringHelper.HtmlEncode(p.Name), StringHelper.HtmlEncode(p.Category), p.Price.ToString().Replace(",", "."), p.Quantity.ToString().Replace(",", ".")
                                              )
                                          ).AggregateString(",")
                    );
