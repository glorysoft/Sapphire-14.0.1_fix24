    ...
    public static class OrderTrafficSourceService
    {
        public static void Add(int objId, TrafficSourceType objType, TrafficSource source)
        {
            SQLDataAccess.ExecuteNonQuery(
                "INSERT INTO [Order].[OrderTrafficSource] ([ObjId],[ObjType],[CustomerId],[CreateOn],[Referrer],[Url],[utm_source],[utm_medium],[utm_campaign],[utm_content],[utm_term],[GoogleClientId],[YandexClientId],[Ip]) " +
                "VALUES (@ObjId,@ObjType,@CustomerId,@CreateOn,@Referrer,@Url,@utm_source,@utm_medium,@utm_campaign,@utm_content,@utm_term,@GoogleClientId,@YandexClientId,@Ip) ",
                CommandType.Text,
                ...
                new SqlParameter("@GoogleClientId", source.GoogleClienId ?? GetGoogleClientId() ?? (object)DBNull.Value),
                new SqlParameter("@YandexClientId", source.YandexClientId ?? GetYandexClientId() ?? (object)DBNull.Value),
                new SqlParameter("@Ip", source.IP ?? (object)DBNull.Value)
            );
        }
