    ...
    public class Avangard : PaymentMethod
    {
        ...
        public override string ProcessServerRequest(Order order)
        {
            ...
            return string.Format(
                "{0}/iacq/pay?ticket={1}", 
                LinkService.PaymentMethod.Avangard,
        ...
        private AvangardResponse GetTicket(Order order)
        {
            ...
            WebRequest request = WebRequest.Create($"{LinkService.PaymentMethod.Avangard}/iacq/h2h/reg?xml={HttpUtility.UrlEncode(postData)}");
            request.Method = "GET";
