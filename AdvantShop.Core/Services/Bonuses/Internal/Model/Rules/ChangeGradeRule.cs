...
namespace AdvantShop.Core.Services.Bonuses.Internal.Model.Rules
{
    ...
    public class ChangeGradeRule : BaseRule
    {
        ...
        {
            if (!InternalBonusSystem.IsEnabled) return;
            ...
            var result = rule.AllPeriod
                ? SQLDataAccess2.Query<ChangeGradeRuleDto>(
                    "select * from( select *, (select top 1 Id from Bonus.Grade g where g.PurchaseBarrier < = temp.PurchaseSum order by PurchaseBarrier DESC ) as NewGradeId " +
                    "from (select card.CardId, card.CardNumber, customer.StandardPhone, card.GradeId, (select ISNULL(sum(pucharse.PurchaseAmount), 0) from Bonus.Purchase pucharse where card.CardId = pucharse.CardId and pucharse.Status = @status) PurchaseSum from Bonus.Card card " +
                    "inner join Customers.Customer customer on card.CardId=customer.CustomerID " +
                    "where card.CardId=@cardId and card.ManualGrade=0 " +
                ...
                : SQLDataAccess2.Query<ChangeGradeRuleDto>(
                    "select * from( select *, (select top 1 Id from Bonus.Grade g where g.PurchaseBarrier < = temp.PurchaseSum order by PurchaseBarrier DESC ) as NewGradeId " +
                    "from (select card.CardId, card.CardNumber, customer.StandardPhone, card.GradeId, (select ISNULL(sum(pucharse.PurchaseAmount), 0) from Bonus.Purchase pucharse where card.CardId = pucharse.CardId and pucharse.Status = @status and pucharse.CreateOnCut>=@minDate) PurchaseSum from Bonus.Card card " +
                    "inner join Customers.Customer customer on card.CardId=customer.CustomerID " +
                    "where card.CardId=@cardId and card.ManualGrade=0 " +
                    "and (select count(*) from Bonus.RuleLog where RuleLog.RuleType=@rule and RuleLog.CardId=card.CardId and RuleLog.Created =@today) < 5" +
                    "group by card.CardId, card.CardNumber, customer.StandardPhone, card.GradeId) temp) t where newGradeId is not null and GradeId <> NewGradeId",
