...
namespace AdvantShop.CMS
{
    public class StaticPageService
    {
        ...
        {
            var staticPage = GetStaticPage(id);
            
            SQLDataAccess.ExecuteNonQuery(
                @"DELETE FROM [CMS].[StaticPage] 
            ...
            CriticalCssService.RemoveByKey(CriticalCssNames.GetStaticPageName(staticPage.UrlPath));
        }
        ...
        public static void UpdateStaticPage(StaticPage page)
        {
            ...
            CriticalCssService.MarkNeedUpdateByKey(CriticalCssNames.GetStaticPageName(page.UrlPath));
        }
        ...
        public static int AddStaticPage(StaticPage page)
        {
	        ...
	        if (page.Meta != null
                && !page.Meta.IsDefaultMeta
                && page.Meta.IsNotEmpty())
	        {
                MetaInfoService.InsertMetaInfo(page.Meta, page.StaticPageId);
	        }
        ...
        public static List<int> GetParentStaticPages(int pageId) =>
            SQLDataAccess.ExecuteReadColumn<int>(
                @"WITH ParentPages AS (                                                       
                    SELECT [StaticPageID], [PageName], [ParentID]
                    FROM [CMS].[StaticPage]
                    WHERE [StaticPageID] = @ChildPageId

                    UNION ALL

                    SELECT sp.[StaticPageID], sp.[PageName], sp.[ParentID]
                    FROM [CMS].[StaticPage] sp
                    INNER JOIN ParentPages pp ON sp.[StaticPageID] = pp.[ParentID]
                )
                SELECT [StaticPageID], [PageName]
                FROM ParentPages", 
                CommandType.Text, 
                "StaticPageID", 
                new SqlParameter("@ChildPageId", pageId)
            );
