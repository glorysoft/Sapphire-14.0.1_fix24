...
namespace AdvantShop.Core
{
    ...
    public class AdvantshopConfigService
    {
        ...
        public static List<ListItemModel> GetDropdownPayments(bool onlyActive = true)
        {
            return CacheManager.Get("GetDropdownPayments-" + onlyActive, () =>
            {
                var payments = new List<ListItemModel>();
                foreach (var payment in GetPayments())
                {
                    if (onlyActive && (payment.Attributes == null || !SQLDataHelper.GetBoolean(payment.Attributes["enabled"].Value)))
                        continue;
                    ...
                    payments = payments
                              .Where(method =>
                                   Saas.SaasDataService.CurrentSaasData.AvailablePaymentTypesList
                                       .Contains(method.Value, StringComparer.OrdinalIgnoreCase))
                              .ToList();
                }
