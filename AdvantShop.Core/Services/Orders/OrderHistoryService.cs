...
namespace AdvantShop.Core.Services.Orders
{
    public class OrderHistoryService
    {
        ...
        public static bool ChangingClosingReceiptStatus(int orderId, EnClosingReceiptStatus status, OrderChangedBy changedBy)
        {
            var oldValue = OrderService.GetClosingReceiptStatus(orderId).Localize();
            var newValue = status.Localize();
            ...
            {
                OrderId = orderId,
                Parameter = LocalizationService.GetResource("Core.Orders.Order.ClosingReceiptStatus"),
                OldValue = oldValue,
                NewValue = newValue
            });
            return true;
        ...
        public static bool ChangingClosingReceiptMessage(int orderId, string message, OrderChangedBy changedBy)
        {
            var oldValue = OrderService.GetClosingReceiptMessage(orderId);
            var newValue = message ?? string.Empty;
            ...
            {
                OrderId = orderId,
                Parameter = LocalizationService.GetResource("Core.Orders.Order.ClosingReceiptMessage"),
                OldValue = oldValue,
                NewValue = newValue
            });
            return true;
        ...
        public static bool ChangingTrackNumber(int orderId, string trackNumber, OrderChangedBy changedBy)
        {
            var oldTrackNumber = OrderService.GetOrderTrackNumber(orderId);
            
            if (oldTrackNumber.Equals(trackNumber))
                return false;

            Add(new OrderHistory(changedBy)
            {
                OrderId = orderId,
                Parameter = LocalizationService.GetResource("Core.Orders.Order.TrackNumber"),
                OldValue = oldTrackNumber,
                NewValue = trackNumber
            });
