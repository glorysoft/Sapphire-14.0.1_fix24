...
namespace AdvantShop.Handlers.Catalog
{
    public class FilterPriceHandler
    {
        ...
        public FilterPriceHandler(float? currentMin, float? currentMax)
        {
            _currentMin = currentMin ?? 0;
            _currentMax = currentMax ?? float.MaxValue;
            _warehouseIds = WarehouseContext.GetAvailableWarehouseIds();
        }

        public FilterPriceHandler(int categoryId, bool indepth, 
                                    float? currentMin, float? currentMax) : this(currentMin, currentMax)
        {
            _categoryId = categoryId;
            _indepth = indepth;
        }
        
        public FilterPriceHandler(EProductOnMain type, int? productListId, 
                                    float? currentMin, float? currentMax) : this(currentMin, currentMax)
        {
        ...
        public FilterPriceHandler(List<int> productIds, float? currentMin, float? currentMax) : this(currentMin, currentMax)
        {
        ...
        {
            var prices = 
                CategoryService.GetPriceRange(_categoryId,
                                                _indepth, 
                                                _productOnMainType,
                                                _productListId,
                                                _productIds,
