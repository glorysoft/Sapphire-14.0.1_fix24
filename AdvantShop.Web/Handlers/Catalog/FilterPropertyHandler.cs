...
namespace AdvantShop.Handlers.Catalog
{
    public class FilterPropertyHandler
    {
        ...
        {
            var showOnlyAvailable = SettingsCatalog.ShowOnlyAvalible;
            
            var propertyValues =
                _productOnMainType == EProductOnMain.None

                    ? CacheManager.Get(CacheNames.PropertiesInCategoryCacheName(_categoryId, _indepth, _productIds, _warehouseIds, showOnlyAvailable),
                        () => PropertyService.GetPropertyValuesByCategories(_categoryId, _indepth, _productIds, _warehouseIds, showOnlyAvailable))

                    : CacheManager.Get(CacheNames.PropertiesInCategoryCacheName((int) _productOnMainType, _productListId, _warehouseIds, showOnlyAvailable),
                        () => PropertyService.GetPropertyValuesByCategories(_productOnMainType, _productListId ?? 0, _warehouseIds, showOnlyAvailable));
