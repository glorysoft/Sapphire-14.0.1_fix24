    ...
    public class NewsPagingHandler
    {
        ...
        public NewsPagingModel Get()
        {
            ...
            _paging = new SqlPaging(CacheNames.News)
                ...
                .From("Settings.News")
                .Left_Join("Catalog.Photo on Photo.objId=News.NewsID and Type={0}", PhotoType.News.ToString())
                .OrderByDesc("AddingDate".AsSqlField("AddingDateSort"))
                .Where("AddingDate <= {0}", DateTime.Now)
                .Where("Enabled = 1");

            if (_newsCategoryId != 0)
                _paging.Where("and NewsCategoryId={0}", _newsCategoryId);

            _paging.ItemsPerPage = _itemPerPage ?? (_currentPageIndex != 0 ? SettingsNews.NewsPerPage : int.MaxValue);
