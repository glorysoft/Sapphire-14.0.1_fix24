...
{
    public class GetProductPropertiesInBriefApi : AbstractCommandHandler<GetProductPropertiesInBriefResponse>
    {
        private readonly int _id;

        public GetProductPropertiesInBriefApi(int id)
        {
            _id = id;
        }
        ...
        protected override GetProductPropertiesInBriefResponse Handle()
        {
            ...
            var result = propertyValues.Select(x => new ProductPropertyApi()
            {
                Name = x.Property.NameDisplayed ?? x.Property.Name ?? "",
                Value = x.Value ?? ""
            }).ToList();

            return new GetProductPropertiesInBriefResponse(result);
        }
    }
}