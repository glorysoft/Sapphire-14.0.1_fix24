...
namespace AdvantShop.Areas.Api.Handlers.BonusLoyalty
{
    public class AcceptBonuses : AbstractCommandHandler<ApiResponse>
    {
        ...
        public AcceptBonuses(Guid customerId, AcceptBonusModel model)
        {
        ...
        {
            return BonusSystem.AddBonuses(_customer, (float)_model.Amount, _model.Reason)
                ? new ApiResponse()
                : new ApiError("Не удалось начислить бонусы");
        }
    }
}