window.addEventListener('load', function load() {
    ...
    const mediaObject = window.matchMedia('only screen and (max-width: 760px), (min-device-width: 768px) and (max-device-width: 1024px)');
    ...
    function process() {
        ...
        for (let i = 0, len = tables.length; i < len; i++) {
            if (tables[i].classList.contains('js-table-responsive-ignore')) {
                ...
                for (let b = 0, lenB = bodyes.length; b < lenB; b++) {
                    rows = bodyes[b].rows;

                    for (let h = 0, lenH = headers.length; h < lenH; h++) {
                    ...
                    if (rows != null && rows.length > 0) {
                        for (let r = 0, lenR = rows.length; r < lenR; r++) {
                            cells = Array.prototype.slice.call(rows[r].cells);

                            if (cells != null && cells.length > 0) {
                                for (let c = 0, lenC = cells.length; c < lenC; c++) {
                                    if (
                                        ignore.indexOf(c) !== -1 ||
    ...
    function generateIndexArray(start, colspan) {
        ...
        for (let i = start; i < colspan + start; i++) {
            array.push(i);
        }
    ...
    function removeCellHeader() {
        const cells = document.querySelectorAll('.js-table-responsive-header-colspan');

        Array.prototype.slice.call(cells).forEach((item) => {
