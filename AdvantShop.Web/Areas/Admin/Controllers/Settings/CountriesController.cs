...
namespace AdvantShop.Web.Admin.Controllers.Settings
{
    ...
    public partial class CountriesController : BaseAdminController
    {
        ...
        public JsonResult EditCountry(CountryModel model)
        {
            if (string.IsNullOrEmpty(model.Name))
            {
                return Json(new { result = false });
        ...
        public JsonResult GetCountryItem(int CountryId)
        {
            ...
            return JsonOk(new CountryModel
            {
                CountryId = country.CountryId,
                DialCode = country.DialCode,
                DisplayInPopup = country.DisplayInPopup,
                Iso2 = country.Iso2,
                Iso3 = country.Iso3,
                Name = country.Name,
        ...
        public JsonResult InplaceCountry(CountryModel model)
        {
        ...
        {
            var result = CountryService.GetAllCountries().Select(x => new SelectItemModel<int>(x.Name, x.CountryId)).ToList();
            return Json(result);
        }
    }
}
