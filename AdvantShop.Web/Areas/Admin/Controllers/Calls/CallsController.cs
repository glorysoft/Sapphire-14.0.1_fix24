    ...
    public partial class CallsController : BaseAdminController
    {
        ...
        public JsonResult CheckDataAccess(int? lastOrderId, int? lastLeadId, int? leadId)
        {
            ...
            if (customer.IsModerator)
            {
                ...
                Order lastOrder;
                if (lastOrderId.HasValue && ((lastOrder = OrderService.GetOrder(lastOrderId.Value)) == null 
                                             || !hasOrdersAccess 
                                             || !OrderService.CheckAccess(lastOrder)
                                             || !OrderService.CheckAccessForEmployeeWithAssignedWarehouses(lastOrder)))
                {
                    lastOrderId = null;
                }
                Lead lastLead;
