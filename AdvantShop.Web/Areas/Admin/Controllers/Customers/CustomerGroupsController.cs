...
namespace AdvantShop.Web.Admin.Controllers.Customers
{
    ...
    public partial class CustomerGroupsController : BaseAdminController
    {
        ...
        public JsonResult GetCustomerGroupCategoryDiscounts(CustomerGroupCategoryDiscountFilter model)
        {
            return Json(new GetCustomerGroupCategoryDiscounts(model).Execute());
        }

        [HttpPost, ValidateJsonAntiForgeryToken]
        public JsonResult InplaceCustomerGroupCategoryDiscount(CustomerGroupCategoryDiscountModel model)
        {
            if (model.Discount == null)
            {
                CustomerGroupService.DeleteCustomerGroupCategoryDiscount(model.CustomerGroupId, model.CategoryId);
            }
            else
            {
                if (model.Discount < 0 || model.Discount > 100)
                    return JsonError("Скидка должна быть от 0 до 100 (%)");
                
                CustomerGroupService.AddOrUpdateCustomerGroupCategoryDiscount(
                    new CustomerGroupCategoryDiscount()
                    {
