...
namespace AdvantShop.Web.Admin.Controllers.Orders
{
    ...
    public partial class GrastinController : BaseAdminController
    {
        ...
        public ActionResult GetFormSendOrderForCdek(int orderId)
        {
            var model = new GrastinGetFormSendOrderForCdek(orderId).Execute();

            if (model == null)
                return new EmptyResult();
            return PartialView("_SendOrderForCdek", model);
        }

        [HttpPost, ValidateJsonAntiForgeryToken]
        public JsonResult SendOrderForCdek(SendOrderForCdekModel model)
        {
            if (ModelState.IsValid)
            {
                var handler = new GrastinSendOrderForCdek(model);
