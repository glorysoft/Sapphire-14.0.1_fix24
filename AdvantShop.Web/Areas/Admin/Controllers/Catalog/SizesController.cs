...
namespace AdvantShop.Web.Admin.Controllers.Catalog
{
    ...
    public class SizesController : BaseAdminController
    {
        ...
        #region Add | Update Size For Categories

        public JsonResult GetSizeNameForCategories(SizesForCategoryFilterModel model)
        {
            return Json(new GetSizeNameForCategoriesHandler(model).Execute());
        }

        [HttpPost, ValidateJsonAntiForgeryToken]
        public JsonResult InplaceCategorySizeName(SizeForCategoryModel model)
        {
            if (model.SizeNameForCategory.IsNullOrEmpty())
                SizeService.DeleteSizeForCategory(model.CategoryId, model.SizeId);
            else
                SizeService.AddUpdateSizeForCategory(model.CategoryId, model.SizeId, model.SizeNameForCategory);
