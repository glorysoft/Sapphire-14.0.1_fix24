...
namespace AdvantShop.Web.Admin.Handlers.Customers.CustomerFields
{
    public class GetCustomerFieldsHandler
    {
        ...
        private void Filter()
        {
            ...
            if (_filterModel.ShowInUserEditing.HasValue)
            {
                _paging.Where("ShowInUserEditing = {0}", _filterModel.ShowInUserEditing.Value);
            }
            ...
            {
                _paging.Where("CustomerType <> 1");
            }
            if (!SettingsCustomers.IsRegistrationAsPhysicalEntity)
            {
                _paging.Where("CustomerType <> 0");
            }
        }
