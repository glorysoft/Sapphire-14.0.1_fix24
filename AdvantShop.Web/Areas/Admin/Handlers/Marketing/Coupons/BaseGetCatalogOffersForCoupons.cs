...
{
    public class BaseGetCatalogOffersForCoupons
    {
        private protected (SqlPaging, FilterResult<IAdminCatalogGridProduct>) GetPaging(
            CouponsCatalogFilterModel filter,
            bool forIds
        )
        {
            ...
            paging
                .Select(
                    ...
                    filter.CouponId.ToString().AsSqlField("CouponId"),
                    "(Select ISNULL(CouponProductOffers.CouponId, 0))".AsSqlField("ApplyCoupon")
                )
                .From("[Catalog].[Product]")
                .Inner_Join(
                    @"(select item, sort 
                    from [Settings].[ParsingBySeperator]({0},'/') ) as productIds 
                        on Product.ProductId = convert(int, productIds.item)",
                ...
                .Left_Join("[Catalog].[CouponProductOffers] ON [Offer].[OfferId] = [CouponProductOffers].[OfferId] and [CouponProductOffers].[CouponId] = " + filter.CouponId);
        ...
        private void Filter(CouponsCatalogFilterModel filter, SqlPaging paging)
        {
            ...
            if (filter.ApplyCoupon != null)
                paging.Where($"CouponProductOffers.CouponId is {(filter.ApplyCoupon.Value ? "not" : "")} null");
