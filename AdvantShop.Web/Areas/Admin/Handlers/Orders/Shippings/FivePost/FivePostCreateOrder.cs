...
{
    public class FivePostCreateOrder
    {
        private readonly int _orderId;

        public FivePostCreateOrder(int orderId)
        {
            _orderId = orderId;
        }
        ...
        public CommandResult Execute()
        {
            ...
            if (shippingMethod.ShippingType != ((ShippingKeyAttribute)typeof(Shipping.FivePost.FivePost).GetCustomAttributes(typeof(ShippingKeyAttribute), false).First()).Value)
            ...
            try
            {
                ...
                var fivePostShipping = new Shipping.FivePost.FivePost(shippingMethod, calculationParameters);
                var result = fivePostShipping.UnloadOrder(order);
            ...
            {
                Debug.Log.Error(ex);
                return new CommandResult() { Error = "Не удалось передать заказ в 5Пост: " + ex.Message };
            }
        }
    }
}
