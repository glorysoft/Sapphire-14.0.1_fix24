...
namespace AdvantShop.Web.Admin.Handlers.Catalog.Reviews
{
    public class GetReviewsHandler
    {
        ...
        private void Filter()
        {
            ...
            if (_filterModel.ShowOnMain != null)
                _paging.Where("ShowOnMain = {0}", Convert.ToInt32(_filterModel.ShowOnMain.Value));
            
            if (_filterModel.HasPhoto != null)
                _paging.Where("PhotoName is " + (_filterModel.HasPhoto.Value ? "not null" : "null"));
            
            if (!string.IsNullOrWhiteSpace(_filterModel.DateFrom) && DateTime.TryParse(_filterModel.DateFrom, out var from) && from.IsValidForSql())
            {
                _paging.Where("AddDate >= {0}", from);
            }
            
            if (!string.IsNullOrWhiteSpace(_filterModel.DateTo) && DateTime.TryParse(_filterModel.DateTo, out var to) && to.IsValidForSql())
            {
