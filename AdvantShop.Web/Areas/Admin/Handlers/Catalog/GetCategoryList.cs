    ...
    public class GetCategoryList
    {
        ...
        public List<AdminCategoryListItemViewModel> Execute()
        {
            ...
            else
            {
                ...
                var categoryIds = CategorySearcherAdmin.Search(_categorySearch).SearchResultItems.Select(item => item.Id)
                    .Union(CategorySearcherAdmin.Search(tanslitQ).SearchResultItems.Select(item => item.Id)).Distinct();
            ...
            return
                categories != null
                    ? categories.Select(x => new AdminCategoryListItemViewModel()
                    {
                        ...
                        MiniPictureSrc = x.MiniPicture != null && x.MiniPicture.PhotoName.IsNotEmpty() ? x.MiniPicture.ImageSrcSmall() : UrlService.GetUrl() + "images/nophoto_xsmall.png",
                        Url = _url.AbsoluteActionUrl("Edit", "Category", new {id = x.CategoryId}),
