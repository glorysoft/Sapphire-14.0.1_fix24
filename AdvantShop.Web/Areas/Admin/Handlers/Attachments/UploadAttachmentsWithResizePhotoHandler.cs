...
{
    public class UploadAttachmentsWithResizePhotoHandler : UploadAttachmentsHandler
    {
        ...
        protected override UploadAttachmentsResult AddAttachment<T>(HttpPostedFile file)
        {
            if (FileHelpers.CheckFileExtensionByType(file.FileName, EFileType.Image | EFileType.Favicon | EFileType.Svg))
            {
                using (Image image = Image.FromStream(file.InputStream))
                {
                    ...
                    FileHelpers.DeleteDirectory(tempDirectoryPath);
                    return base.AddAttachment<T>(newFile);
                }
            }
            return base.AddAttachment<T>(file);
        }
