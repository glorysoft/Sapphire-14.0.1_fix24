    ...
    public class GetTaskGroupsHandler
    {
        ...
        private void GetPaging()
        {
            ...
            _paging.Select(
                ...
                "(SELECT COUNT(1) FROM Customers.Task INNER JOIN Customers.ProjectStatus ps ON StatusId = ps.Id WHERE TaskGroupId = TaskGroup.Id AND ps.StatusType = 0)".AsSqlField("NewTaskCount"),
                "(SELECT COUNT(1) FROM Customers.Task INNER JOIN Customers.ProjectStatus ps ON StatusId = ps.Id WHERE TaskGroupId = TaskGroup.Id AND ps.StatusType = 1)".AsSqlField("InProgressTaskCount"),
                "(SELECT COUNT(1) FROM Customers.Task INNER JOIN Customers.ProjectStatus ps ON StatusId = ps.Id WHERE TaskGroupId = TaskGroup.Id AND ps.StatusType = 2)".AsSqlField("CompletedTaskCount"),
                "(SELECT COUNT(1) FROM Customers.Task INNER JOIN Customers.ProjectStatus ps ON StatusId = ps.Id WHERE TaskGroupId = TaskGroup.Id AND ps.StatusType = 3)".AsSqlField("AcceptedTaskCount"),
                "(SELECT COUNT(1) FROM Customers.Task INNER JOIN Customers.ProjectStatus ps ON StatusId = ps.Id WHERE TaskGroupId = TaskGroup.Id AND ps.StatusType = 4)".AsSqlField("CanceledTaskCount")
                );
