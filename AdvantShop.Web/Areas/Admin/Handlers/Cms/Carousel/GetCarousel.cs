...
namespace AdvantShop.Web.Admin.Handlers.Cms.Carousel
{
    public class GetCarousel
    {
        ...
        public List<int> GetItemsIds()
        {
            ...
            return _paging.ItemsIds<int>("Carousel.CarouselID");
        }
        ...
        private void GetPaging()
        {
            ...
            _paging.Select(
                ...
                "PhotoName",
                "VideoName",
                "CarouselText.Title".AsSqlField("TextTitle"));
            ...
            _paging.Left_Join("CMS.CarouselVideo on CarouselVideo.CarouselId=Carousel.CarouselID");
            _paging.Outer_Apply("(SELECT TOP(1) Title FROM [CMS].[CarouselText] WHERE CarouselText.CarouselId = Carousel.CarouselID) CarouselText");
        ...
        private void Filter()
        {
            ...
            if (!string.IsNullOrWhiteSpace(_filterModel.TextTitle))
            {
                _paging.Where("EXISTS (SELECT 1 FROM [CMS].[CarouselText] ct WHERE " +
                              "ct.CarouselId = Carousel.CarouselID AND ct.Title LIKE '%'+{0}+'%')", _filterModel.TextTitle
                );
            }
        }
