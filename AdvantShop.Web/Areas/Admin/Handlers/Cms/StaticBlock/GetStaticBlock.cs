...
namespace AdvantShop.Web.Admin.Handlers.Cms.StaticBlock
{
    ...
    {
        private readonly StaticBlockFilterModel _filterModel;
        ...
        private void Filter()
        {
            ...
            {
                _paging.Where("[Key] LIKE '%'+{0}+'%' OR InnerName LIKE '%'+{0}+'%' OR [Content] LIKE '%'+{0}+'%'", _filterModel.Search);
            }
            ...
            if (!string.IsNullOrWhiteSpace(_filterModel.Content))
            {
                _paging.Where("[Content] LIKE '%'+{0}+'%'", _filterModel.Content);
            }
        }
