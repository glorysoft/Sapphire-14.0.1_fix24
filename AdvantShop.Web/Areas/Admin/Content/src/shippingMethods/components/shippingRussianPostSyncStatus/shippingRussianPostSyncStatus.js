...
(function (ng) {
    ...
    const ShippingRussianPostSyncStatusCtrl = function ($http, toaster) {
        const ctrl = this;
        ctrl.statuses = {
            s1: 'Прием',
        ...
        ctrl.addSyncStatus = function () {
            const arr = ctrl.Status.slice(1).split('_');
            const status = arr[0];
            const attribut = arr.length === 1 ? null : arr[1];
            if (
                ctrl.syncStatuses.some((item) => item.rpStatus === status && item.rpAttribut === attribut)
            ) {
        ...
        ctrl.updateStatusesReference = function () {
            ctrl.statusesReference = ctrl.syncStatuses
                .map((x) => `${x.rpStatus + (x.rpAttribut ? `_${  x.rpAttribut}` : '')  },${  x.advStatus}`)
                .join(';');
            ctrl.update = true;
        };
        ...
        function compare(a, b) {
            ...
            if (a.rpStatus > b.rpStatus) return 1;
            const arpAttribut = a.rpAttribut == null ? -1 : a.rpAttribut;
            const brpAttribut = b.rpAttribut == null ? -1 : b.rpAttribut;
