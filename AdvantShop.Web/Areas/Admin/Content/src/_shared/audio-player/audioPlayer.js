(function (ng) {
    ...
    const AudioPlayerCtrl = function ($http, $element, $scope) {
        const ctrl = this;
        ...
        ctrl.initAudio = function (src) {
            ...
            ctrl.audio.addEventListener('error', () => {
                ctrl.noAudio = true;
                ctrl.isplaying = false;
                ctrl.audio = null;
                $scope.$apply();
        ...
        ctrl.setTime = function ($event) {
            ...
            const position = $event.clientX - $event.target.getBoundingClientRect().left;
            const percent = (position / ctrl.timelineWidth) * 100;
        ...
        function timeupdate() {
            ...
            const percent = Math.ceil((ctrl.audio.currentTime / ctrl.audio.duration) * 100);
