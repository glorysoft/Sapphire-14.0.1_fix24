...
(function ($) {
    ...
    $.ThreeSixty = function (el, options) {
        ...
        base.fSBackgroundColor = function () {
            const bgColor = AppConfig.fSBackgroundColor;
            return bgColor;
        };
        ...
        base.loadImages = function () {
            let li, imageName, image, host, baseIndex;
        ...
        base.initPlugins = function () {
            $.each(AppConfig.plugins, (i, plugin) => {
        ...
        base.showNavigation = function () {
            if (AppConfig.navigation && !AppConfig.navigation_init) {
                let nav_bar, next_btn, next, previous_btn, previous, play_stop_btn, play_stop, full_btn, full, position;
        ...
        base.play = function (speed, direction) {
            const _speed = speed || AppConfig.playSpeed;
            const _direction = direction || AppConfig.autoplayDirection;
        ...
        base.gotoAndPlay = function (n) {
            if (AppConfig.disableWrap) {
                ...
                let multiplier = Math.ceil(AppConfig.endFrame / AppConfig.totalFrames);
                if (multiplier === 0) {
                    multiplier = 1;
                }

                // Figure out the quickest path to the requested frame
                const realEndFrame = multiplier > 1 ? AppConfig.endFrame - (multiplier - 1) * AppConfig.totalFrames : AppConfig.endFrame;

                const currentFromEnd = AppConfig.totalFrames - realEndFrame;
        ...
        base.initEvents = function () {
            base.$el.bind('mousedown mousemove click', (event) => {
                event.preventDefault();
            ...
            base.$el.bind('touchstart touchmove touchend', (event) => {
            ...
            $(document).bind('mousemove', (event) => {
                ...
                } else if (!base.browser.isIE && AppConfig.showCursor) {
        ...
        base.zeroPad = function (num) {
            ...
            const approximateLog = Math.log(AppConfig.totalFrames) / Math.LN10;
            const roundTo = 1e3;
            const roundedLog = Math.round(approximateLog * roundTo) / roundTo;
            const numChars = Math.floor(roundedLog) + 1;
