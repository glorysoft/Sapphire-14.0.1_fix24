(function (ng) {
    ...
    function getType(path) {
        return ['.jpg', '.png', '.gif', '.bmp'].some((val) => path.toLowerCase().indexOf(val) !== -1);
    }
        ...
        function (magnificPopupDefault, $parse) {
            return {
                ...
                link (scope, element, attrs, ctrl) {
                    const delegate = element[0].nodeName === 'A' ? null : 'a';
                    ...
                    if (attrs.plugin === 'fancybox') {
                        ...
                        if (element[0].nodeName == 'A') {
                            ...
                            let href = element[0].getAttribute('href'),
                            ...
                            if (rel != null && rel.length > 0) {
                                ...
                                Array.prototype.slice.call(itemsForGroup).forEach((item) => {
                                    itemsObj.push({
                                        type: 'image',
                        ...
                        const customOptions = $parse(attrs.magnificPopupOptions)(scope);
                        const resultOptions = angular.extend(
                            {
                                delegate,
                                type: 'image',
